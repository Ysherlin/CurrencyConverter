...
namespace Currency_Converter
{
	...
	public partial class MainWindow : Window
	{
		...
		{
            val = await GetData<Root>("https://openexchangerates.org/api/latest.json?app_id=3f7f1a53151e44428fe74ff3baf99bff");
            BindCurrency();
		}
        ...
        public static async Task<Root> GetData<T>(string url)
		{
			...
			try
			{
                ...
                using(var client = new HttpClient())
				{
                    ...
                    //Check API response status ok
                    if(response.StatusCode == System.Net.HttpStatusCode.OK)
					{
                        //Serialize the HTTP content to a string as an asynchronous operation.
                        var ResponseString = await response.Content.ReadAsStringAsync();
                        //JsonConvert.DeserializeObject to DeserializeObject to deserialize Json to a C#
